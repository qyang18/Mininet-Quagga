...
if __name__ == "__main__":
    ...
    p = plt
    p.title('Plot of Two Mininet Two Way Test With Physical Cable in Hybrid mode')
    # p.figure(1)
    # for i in range(1,7):
        # file.append(getLogFileInCDF('log'+str(i)))
    file1.append(getLogFile('./TwoMininetTestServer'))
    # plot(p,file1,1,7,'r','Mix CenterPhysical',1)

    plot(p,file1,1,1,'r','TwoMininetTest H1',1)

    file2.append(getLogFile('./TwoMininetTestClient'))
    plot(p, file2, 1, 1, 'b', 'TwoMininetTest H2', 1)
